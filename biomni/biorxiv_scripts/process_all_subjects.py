...
BIORXIV_SUBJECTS = [
    ...
    'developmental biology', 'plant biology', 'microbiology',
    'cancer biology', 'immunology', 'cell biology', 'biochemistry',
    'genetics', 'bioinformatics', 'animal behavior and cognition',
    'biophysics', 'genomics', 'systems biology', 'bioengineering',
    'molecular biology', 'physiology', 'zoology',
    'scientific communication and education', 'pathology',
    'synthetic biology', 'paleontology', 'pharmacology and toxicology'
]

def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Process all bioRxiv subjects and extract tasks from papers.')
    parser.add_argument('--papers-per-subject', type=int, default=100, 
                        help='Number of papers to process per subject (default: 100)')
    parser.add_argument('--metadata-path', type=str, default='/dfs/user/kexinh/BioAgentOS/data/biorxiv_metadata.csv', 
                        help='Path to bioRxiv metadata CSV file (default: data/biorxiv_metadata.csv)')
    parser.add_argument('--output-dir', type=str, default='./biorxiv_all_subjects', 
    ...
    parser.add_argument('--max-paper-length', type=int, default=200000, 
    ...
    parser.add_argument('--save-pdfs', action='store_true', 
                        help='Save downloaded PDFs (default: False)')
...
def check_subject_availability(metadata_path, subjects):
    ...
    try:
        ...
        for subject in subjects:
            count = len(df_biorxiv[(df_biorxiv.published != 'NA') & 
...
def process_subject(subject, args):
    ...
    # Build command
    cmd = [
        "python", "BioAgentOS/bioagentos/scripts/extract_biorxiv_tasks.py",
        "--subject", f'"{subject}"',
        "--limit", str(args.papers_per_subject),
        "--metadata-path", args.metadata_path,
        "--model", args.model,
        "--max-paper-length", str(args.max_paper_length),
...
def combine_summaries(base_dir, subjects, papers_per_subject):
    ...
    all_tasks = {k: v for k, v in sorted(all_tasks.items(), key=lambda item: item[1], reverse=True)}
    all_databases = {k: v for k, v in sorted(all_databases.items(), key=lambda item: item[1], reverse=True)}
    all_software = {k: v for k, v in sorted(all_software.items(), key=lambda item: item[1], reverse=True)}
...
def main():
    ...
    if not args.summary_only:
        ...
        for subject, count in tqdm(available_subjects, desc="Processing subjects"):
